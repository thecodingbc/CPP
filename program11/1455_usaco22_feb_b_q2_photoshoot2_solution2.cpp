...
int main() {
    ...
    */
    ...
    for(int i = 0; i < n; i++) {
        ...
        // we need to compare all numbers in *FRONT* of no_i, see whether any one's position goes *AFTER* i in target
        for(int j = 0; j < i; j++) {

            int no_j = original[j];
            int no_j_position = positions[no_j];

            if(no_j_position > no_i_position) {
                count++; // no_i needs to be moved to the front
                break;
            }
        }
    }
